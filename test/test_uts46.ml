(** UTS #46 conformance against IdnaTestV2.txt.
    ...
    Format per entry: source ; toUnicode ; toUnicodeStatus ; toAsciiN ;
    toAsciiNStatus ; toAsciiT ; toAsciiTStatus

    Blank toUnicode = same as source. Blank toAsciiN = same as toUnicode. Blank
    status = no errors. Explicit [] = no errors. Literal "" = empty string. *)
...
let check_to_unicode_case ?(flags = Idna.Uts46.default_flags) source
...
let check_to_ascii_ok ?(flags = Idna.Uts46.default_flags) source expected_value
  ...
  | Error msg ->
      Alcotest.fail
        (Printf.sprintf "toAscii(%s): expected Ok, got Error %S"
           (label_of source) msg)
...
let check_to_ascii_error ?(flags = Idna.Uts46.default_flags) source =
  ...
  | Ok value ->
      Alcotest.fail
        (Printf.sprintf "toAscii(%s): expected Error, got Ok %S"
           (label_of source) value)
  | Error _ -> ()

let test_regression_invalid_utf8_ascii_lowercase () =
  check_to_unicode_case "A\xed\xa4\x80Z" "a\xed\xa4\x80z" true

let test_regression_xn_empty () = check_to_unicode_case "xn--" "" true
...
let () =
  Alcotest.run "uts46"
    [
      ( "regressions",
        [
          Alcotest.test_case "invalid utf8 lowercases ascii" `Quick
            test_regression_invalid_utf8_ascii_lowercase;
          Alcotest.test_case "xn-- to empty" `Quick test_regression_xn_empty;
          Alcotest.test_case "xn-- ascii hyphen" `Quick
            test_regression_xn_ascii_hyphen;
          Alcotest.test_case "xn-- unicode hyphen domain" `Quick
            test_regression_xn_unicode_hyphen_domain;
          Alcotest.test_case "root label verify disabled" `Quick
            test_regression_root_label_verify_disabled;
          Alcotest.test_case "root label verify enabled" `Quick
            test_regression_root_label_verify_enabled;
          Alcotest.test_case "reserved xn when hyphen checks disabled" `Quick
            test_regression_reserved_xn_when_check_hyphens_disabled;
        ] );
