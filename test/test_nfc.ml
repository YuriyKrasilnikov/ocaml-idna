(** NFC conformance against Unicode NormalizationTest.txt (UAX #15).
    ...
    For each test row c1;c2;c3;c4;c5: toNFC(c1) = toNFC(c2) = toNFC(c3) = c2
...
let load_rows path =
  ...
  (try
     while true do
       ...
       if String.length line > 0 && line.[0] = '@' then part := line
...
let () =
  Alcotest.run "nfc"
    [ ("NormalizationTest", [ Alcotest.test_case "all rows" `Quick test_nfc ]) ]
