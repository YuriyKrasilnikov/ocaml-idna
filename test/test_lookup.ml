...
let check_ascii_ok ?(flags = Idna.Lookup.default_flags) input expected =
  ...
  | Error msg ->
      Alcotest.fail
        (Printf.sprintf "Lookup.to_ascii %S: expected Ok %S, got Error %S" input
           expected msg)
...
let check_ascii_err ?(flags = Idna.Lookup.default_flags) input =
  ...
  | Ok actual ->
      Alcotest.fail
        (Printf.sprintf "Lookup.to_ascii %S: expected Error, got Ok %S" input
           actual)
...
let test_disallowed_rejected_for_lookup () = check_ascii_err "a\xe2\x80\xa8"
let test_contextj_enforced_for_lookup () = check_ascii_err "a\xe2\x80\x8ca"
...
let () =
  Alcotest.run "lookup"
    [
      ( "lookup",
        [
          Alcotest.test_case "apparent a-label lowercased" `Quick
            test_apparent_alabel_lowercased;
          Alcotest.test_case "ascii nr-ldh lowercased" `Quick
            test_ascii_nr_ldh_lowercased;
          Alcotest.test_case "disallowed rejected" `Quick
            test_disallowed_rejected_for_lookup;
          Alcotest.test_case "contextj enforced" `Quick
            test_contextj_enforced_for_lookup;
          Alcotest.test_case "contexto relaxed" `Quick
            test_contexto_relaxed_for_lookup;
          Alcotest.test_case "bidi default rejects digit label after rtl" `Quick
            test_bidi_default_rejects_digit_label_after_rtl;
          Alcotest.test_case "bidi can be disabled" `Quick
            test_bidi_can_be_disabled;
          Alcotest.test_case "dns length not prechecked for long label" `Quick
            test_dns_length_not_prechecked_for_long_label;
          Alcotest.test_case "dns length not prechecked for long domain" `Quick
            test_dns_length_not_prechecked_for_long_domain;
        ] );
    ]
