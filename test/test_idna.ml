...
let check_err label =
  match Idna.Registration.check_label label with
  | Ok () ->
      Alcotest.fail (Printf.sprintf "check_label %S: expected error" label)
  | Error _ -> ()
...
let check_err_msg label expected_prefix =
  ...
  | Error msg ->
      if
        not
          (String.length msg >= String.length expected_prefix
          && String.sub msg 0 (String.length expected_prefix) = expected_prefix
          )
      then
        Alcotest.fail
          (Printf.sprintf "check_label %S: expected %S prefix, got %S" label
             expected_prefix msg)

let check_result_err name = function
  | Ok value ->
      Alcotest.fail (Printf.sprintf "%s: expected error, got %S" name value)
  | Error _ -> ()
...
let valid s =
  Alcotest.(check bool)
    ...
    true
    (Idna.Registration.is_valid_hostname s)
...
(* xn-- labels (A-label) *)
let test_xn_valid () = check_ok "xn--maana-pta" (* mañana *)
...
let test_ulabel () = check_ok "\xc3\xb1" (* ñ, U+00F1, PVALID, bidi_l *)

(* 4-byte UTF-8 codepoint: U+20000 CJK Extension B *)
let test_4byte_utf8 () = check_ok "\xf0\xa0\x80\x80"

(* ══════════════════════════════════════════════════════════════ *)
(* 2. Negative: check_label length errors                        *)
(* ══════════════════════════════════════════════════════════════ *)

let test_empty_label () = check_err_msg "" "empty label"

(* check_label does not enforce DNS length — is_valid_hostname does *)
let test_label_too_long () =
  Alcotest.(check bool)
    "64-char rejected by hostname" false
    (Idna.Registration.is_valid_hostname (String.make 64 'a'))

(* ══════════════════════════════════════════════════════════════ *)
(* 3. Hyphen rules                                               *)
(* ══════════════════════════════════════════════════════════════ *)

let test_hyphen_start () = check_err_msg "-abc" "label starts with hyphen"
let test_hyphen_end () = check_err_msg "abc-" "label ends with hyphen"
let test_hyphen_34 () = check_err_msg "ab--cd" "label has -- at positions 3-4"
let test_hyphen_internal_ok () = check_ok "a-b"
...
let test_initial_combiner_alabel () = check_err "xn--ced"
...
let test_nfc_qc_no () = check_err_msg "a\xcd\x84" "label not in NFC"
...
let test_disallowed_separator () = check_err "a\xe2\x80\xa8"

let test_surrogate_utf8_public_barrier () =
  let surrogate = "\xED\xA0\x80" in
  check_err_msg surrogate "invalid UTF-8";
  check_result_err "Registration.to_ascii"
    (Idna.Registration.to_ascii surrogate);
  check_result_err "Registration.to_unicode"
    (Idna.Registration.to_unicode surrogate);
  check_result_err "Lookup.to_ascii" (Idna.Lookup.to_ascii surrogate);
  check_result_err "Lookup.to_unicode" (Idna.Lookup.to_unicode surrogate);
  check_result_err "Uts46.to_ascii" (Idna.Uts46.to_ascii surrogate);
  let unicode = Idna.Uts46.to_unicode surrogate in
  Alcotest.(check bool) "Uts46.to_unicode errored" true unicode.errored
...
(* Valid: between two 'l' *)
let test_middle_dot_valid () = check_ok "l\xc2\xb7l"

(* Invalid: not between 'l' *)
let test_middle_dot_wrong_neighbors () = check_err "a\xc2\xb7a"

(* Invalid: at start of label *)
let test_middle_dot_at_start () = check_err "\xc2\xb7l"

(* Invalid: at end of label *)
let test_middle_dot_at_end () = check_err "l\xc2\xb7"
...
let test_greek_numeral_valid () = check_ok "\xce\xb1\xcd\xb5\xce\xb1"

(* Invalid: at end of label — no next char *)
let test_greek_numeral_at_end () = check_err "\xce\xb1\xcd\xb5"
...
let test_hebrew_geresh_valid () = check_ok "\xd7\x90\xd7\xb3"

(* Valid: Hebrew ALEF + GERSHAYIM *)
let test_hebrew_gershayim_valid () = check_ok "\xd7\x90\xd7\xb4"

(* Invalid: GERESH at start — no preceding Hebrew *)
let test_hebrew_geresh_at_start () = check_err "\xd7\xb3\xd7\x90"
...
let test_katakana_dot_with_katakana () = check_ok "\xe3\x83\xbb\xe3\x82\xa2"

(* Valid: with Hiragana A (U+3042) *)
let test_katakana_dot_with_hiragana () = check_ok "\xe3\x83\xbb\xe3\x81\x82"

(* Valid: with CJK Han (U+4E00) *)
let test_katakana_dot_with_han () = check_ok "\xe3\x83\xbb\xe4\xb8\x80"

(* Invalid: alone *)
let test_katakana_dot_alone () = check_err "xn--vek"
...
let test_arabic_indic_valid () = check_ok "\xd8\xa8\xd9\xa0\xd8\xa8"

(* Invalid: mixed with Extended Arabic-Indic *)
(* U+06F0 = \xdb\xb0 *)
let test_arabic_indic_mixed () = check_err "\xd8\xa8\xd9\xa0\xdb\xb0\xd8\xa8"
...
let test_ext_arabic_valid () = check_ok "\xd8\xa8\xdb\xb0\xd8\xa8"

(* Invalid: mixed with Arabic-Indic *)
let test_ext_arabic_mixed () = check_err "\xd8\xa8\xdb\xb0\xd9\xa0\xd8\xa8"
...
let test_zwnj_joining_context () = check_ok "\xd8\xa8\xe2\x80\x8c\xd8\xa7"
...
let test_zwj_no_virama () = check_err "a\xe2\x80\x8da"

(* Invalid: ZWNJ without virama and without joining context *)
let test_zwnj_no_context () = check_err "a\xe2\x80\x8ca"

(* Invalid: ZWNJ at start — no left context *)
let test_zwnj_at_start () = check_err "\xe2\x80\x8c\xd8\xa8"

(* Invalid: ZWNJ with left but no right *)
let test_zwnj_no_right () = check_err "\xd8\xa8\xe2\x80\x8c"
...
let test_bidi_rtl_valid () = check_ok "\xd7\x90\xd7\x91"
...
let test_bidi_rtl_nsm_end () = check_ok "\xd7\x90\xd7\x91\xd6\xb0"
...
let test_bidi_ltr_with_rtl () = check_err "a\xd7\x90"

(* Single RTL character *)
let test_bidi_single_rtl () = check_ok "\xd7\x90"

(* ══════════════════════════════════════════════════════════════ *)
(* 15. check_label: xn-- with invalid punycode                   *)
(* ══════════════════════════════════════════════════════════════ *)

let test_xn_invalid_punycode () = check_err_msg "xn--!!!" "invalid punycode"

(* xn-- that decodes to DISALLOWED codepoint *)
let test_xn_disallowed () = check_err "xn--chb89f"
...
let test_max_label () = check_ok (String.make 63 'a')
let test_single_char () = check_ok "a"
...
let test_hostname_too_long () = invalid (String.make 254 'a')
...
let test_hostname_bidi_digit_after_rtl () = valid "\xd7\x90\xd7\x91.1com"
    ...
    [
      ( "check_label:happy",
        [
          ...
          Alcotest.test_case "xn-- uppercase rejected" `Quick
            test_xn_uppercase_rejected;
      ...
      ( "check_label:disallowed",
        [
          ...
          Alcotest.test_case "surrogate utf8 public barrier" `Quick
            test_surrogate_utf8_public_barrier;
        ] );
      ( "contexto:middle_dot",
        [
          Alcotest.test_case "valid l·l" `Quick test_middle_dot_valid;
          Alcotest.test_case "wrong neighbors" `Quick
            test_middle_dot_wrong_neighbors;
      ...
      ( "contexto:hebrew",
        [
          ...
          Alcotest.test_case "gershayim valid" `Quick
            test_hebrew_gershayim_valid;
          Alcotest.test_case "geresh at start" `Quick
            test_hebrew_geresh_at_start;
        ] );
      ( "contexto:katakana",
        [
          Alcotest.test_case "with katakana" `Quick
            test_katakana_dot_with_katakana;
          Alcotest.test_case "with hiragana" `Quick
            test_katakana_dot_with_hiragana;
      ...
      ( "contextj",
        [
          ...
          Alcotest.test_case "zwnj joining+T" `Quick
            test_zwnj_joining_with_transparent;
      ...
      ( "xn--",
        [
          ...
          Alcotest.test_case "non-canonical round-trip" `Quick
            test_xn_noncanonical;
        ] );
      ( "boundary",
        [
      ...
      ( "is_valid_hostname",
        [
          ...
          Alcotest.test_case "bidi digit after rtl" `Quick
            test_hostname_bidi_digit_after_rtl;
        ] );
    ]
