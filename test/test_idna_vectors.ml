...
let () =
  ...
  List.iter
    (fun v ->
      ...
      let has_nv8 =
        Test_helper.string_has_nv8_xv8 nv8_ranges to_unicode_resolved
      in
      incr total;
      ...
      if our_invalid = expected_invalid then incr pass
      else begin
        incr fail;
        let cat =
  ...
  List.iter
    (fun v ->
      ...
      if
        (not v.Test_helper.to_unicode_err)
      ...
      then begin
        incr enc_total;
        match
          Idna.Registration.to_ascii ~flags:reg_flags to_unicode_resolved
        with
        | Ok actual
          when String.lowercase_ascii actual
               = String.lowercase_ascii to_ascii_n_decoded ->
            incr enc_pass
        | Ok actual ->
            incr enc_fail;
  ...
  Printf.printf "  rate:   %.1f%%\n"
    (if !total > 0 then 100.0 *. float_of_int !pass /. float_of_int !total
     else 0.0);
  if Hashtbl.length fail_by_cat > 0 then begin
    Printf.printf "\nFailures by category:\n";
    Hashtbl.iter
      (fun cat count -> Printf.printf "  %-15s %d\n" cat count)
      fail_by_cat
  end;
  ...
  Printf.printf "  rate:   %.1f%%\n"
    (if !enc_total > 0 then
       100.0 *. float_of_int !enc_pass /. float_of_int !enc_total
     else 0.0);
