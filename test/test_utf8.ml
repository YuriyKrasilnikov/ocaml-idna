...
let check_err name input =
  ...
  | Ok cps ->
      Alcotest.fail
        (Printf.sprintf "%s: expected Error, got Ok %s" name
           (String.concat "," (List.map string_of_int cps)))
...
let test_truncated_3byte_2 () =
  check_err "truncated 3-byte missing 1" "\xE4\xB8"

let test_truncated_4byte_1 () = check_err "truncated 4-byte missing 3" "\xF0"

let test_truncated_4byte_2 () =
  check_err "truncated 4-byte missing 2" "\xF0\x9F"

let test_truncated_4byte_3 () =
  check_err "truncated 4-byte missing 1" "\xF0\x9F\x98"
...
let test_overlong_null_2byte () =
  check_err "overlong U+0000 as 2-byte" "\xC0\x80"
...
let test_overlong_7F_3byte () =
  check_err "overlong U+007F as 3-byte" "\xE0\x81\xBF"
...
let test_invalid_continuation_3byte () =
  check_err "invalid cont 3-byte" "\xE4\x00\x80"
...
let test_of_cps_rejects_non_scalar () =
  let check cps =
    match Idna.Utf8.of_cps cps with
    ...
    [
      ...
      ( "roundtrip",
        [
          ...
          Alcotest.test_case "of_cps non-scalar" `Quick
            test_of_cps_rejects_non_scalar;
        ] );
    ]
