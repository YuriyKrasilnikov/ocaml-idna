...
let unquote_field s = if s = "\"\"" then "" else s
...
let decode_escapes s =
  ...
  while !i < len do
    if !i + 5 < len && s.[!i] = '\\' && s.[!i + 1] = 'u' then begin
      ...
      !i + 3 < len && s.[!i] = '\\' && s.[!i + 1] = 'x' && s.[!i + 2] = '{'
    then
      begin match String.index_from_opt s (!i + 3) '}' with
      | Some close -> (
...
let string_has_nv8_xv8 ranges s =
  ...
  List.exists
    (fun cp -> List.exists (fun (lo, hi) -> cp >= lo && cp <= hi) ranges)
    cps
