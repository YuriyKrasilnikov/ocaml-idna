(** Systematic tests for CONTEXTO rules (RFC 5892 Appendix A.3-A.9).
    ...
    For each rule, one positive case satisfying the Lookup formula and negative
    cases each violating one condition of the formula. *)

let check_ok name utf8 =
  match Idna.Registration.check_label utf8 with
  | Ok () -> ()
  | Error e ->
      Alcotest.fail (Printf.sprintf "%s: expected Ok, got Error %S" name e)
...
let l = 0x6C (* 'l' *)
let a = 0x61 (* 'a' *)
let a3_positive () = check_ok "A.3 l·l" (utf8_of_cps [ l; middle_dot; l ])
...
let a3_alone () = check_err "A.3 alone" (utf8_of_cps [ middle_dot ])
...
let a4_positive () = check_ok "A.4 keraia+Greek" (utf8_of_cps [ keraia; alpha ])
...
let a4_alone () = check_err "A.4 keraia alone" (utf8_of_cps [ keraia ])
...
let a5_alone () = check_err "A.5 geresh alone" (utf8_of_cps [ geresh ])
...
let a6_alone () = check_err "A.6 gershayim alone" (utf8_of_cps [ gershayim ])
...
let a7_positive_script_before () =
  check_ok "A.7 Hiragana+dot+Latin"
    (utf8_of_cps [ hiragana_a; katakana_dot; a ])
...
let a7_alone () = check_err "A.7 dot alone" (utf8_of_cps [ katakana_dot ])
...
let a8_positive_each_digit () =
  List.iter
    (fun cp ->
      check_ok
        (Printf.sprintf "A.8 alef+U+%04X" cp)
...
let a8_negative_mixed_with_extended () =
  List.iter
    (fun ext ->
      check_err
        (Printf.sprintf "A.8 alef+0660+U+%04X" ext)
...
let a9_positive_each_digit () =
  List.iter
    (fun cp ->
      check_ok
        (Printf.sprintf "A.9 alef+U+%04X" cp)
        (utf8_of_cps [ arabic_alef; cp ]))
    extended_arabic_range
...
let a9_negative_mixed_with_arabic () =
  List.iter
    (fun ar ->
      check_err
        (Printf.sprintf "A.9 alef+06F0+U+%04X" ar)
...
let adversarial_many_middle_dots () =
  check_ok "A.3 many valid middle dots"
    (utf8_of_cps (repeat 128 [ l; middle_dot ] @ [ l ]))

let adversarial_many_katakana_dots () =
  check_ok "A.7 many dots with one Katakana witness"
    (utf8_of_cps (katakana_a :: repeat 256 [ katakana_dot ]))
    ...
    [
      ( "A.3 MIDDLE DOT",
        [
          ...
          Alcotest.test_case "no before (first pos)" `Quick
            a3_no_before_first_pos;
      ...
      ( "A.4 GREEK KERAIA",
        [
          ...
          Alcotest.test_case "after non-Greek Latin" `Quick
            a4_after_non_greek_latin;
          Alcotest.test_case "after non-Greek Hebrew" `Quick
            a4_after_non_greek_hebrew;
      ...
      ( "A.5 HEBREW GERESH",
        [
          ...
          Alcotest.test_case "before non-Hebrew Latin" `Quick
            a5_before_non_hebrew_latin;
          Alcotest.test_case "before non-Hebrew Greek" `Quick
            a5_before_non_hebrew_greek;
          Alcotest.test_case "no before (first pos)" `Quick
            a5_no_before_first_pos;
      ...
      ( "A.6 HEBREW GERSHAYIM",
        [
          ...
          Alcotest.test_case "before non-Hebrew Latin" `Quick
            a6_before_non_hebrew_latin;
          Alcotest.test_case "before non-Hebrew Greek" `Quick
            a6_before_non_hebrew_greek;
          Alcotest.test_case "no before (first pos)" `Quick
            a6_no_before_first_pos;
          Alcotest.test_case "alone" `Quick a6_alone;
        ] );
      ( "A.7 KATAKANA MIDDLE DOT",
        [
          Alcotest.test_case "positive with Hiragana" `Quick
            a7_positive_hiragana;
          Alcotest.test_case "positive with Katakana" `Quick
            a7_positive_katakana;
          Alcotest.test_case "positive with Han" `Quick a7_positive_han;
          Alcotest.test_case "positive script before dot" `Quick
            a7_positive_script_before;
          Alcotest.test_case "positive script far from dot" `Quick
            a7_positive_script_far;
          Alcotest.test_case "no required script (Latin)" `Quick
            a7_no_required_script_all_latin;
          Alcotest.test_case "no required script (Greek)" `Quick
            a7_no_required_script_greek;
          Alcotest.test_case "alone" `Quick a7_alone;
        ] );
      ( "A.8 ARABIC-INDIC DIGITS",
        [
          Alcotest.test_case "each digit after alef" `Quick
            a8_positive_each_digit;
          Alcotest.test_case "all digits together" `Quick
            a8_positive_all_together;
          Alcotest.test_case "mixed with each Extended digit" `Quick
            a8_negative_mixed_with_extended;
        ] );
      ( "A.9 EXTENDED ARABIC-INDIC DIGITS",
        [
          Alcotest.test_case "each digit after alef" `Quick
            a9_positive_each_digit;
          Alcotest.test_case "all digits together" `Quick
            a9_positive_all_together;
          Alcotest.test_case "mixed with each Arabic-Indic digit" `Quick
            a9_negative_mixed_with_arabic;
        ] );
      ( "adversarial",
        [
          Alcotest.test_case "many valid middle dots" `Quick
            adversarial_many_middle_dots;
          Alcotest.test_case "many katakana dots" `Quick
            adversarial_many_katakana_dots;
        ] );
    ]
