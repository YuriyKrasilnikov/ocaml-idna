...
let string_of_cps cps =
  cps |> List.map (Printf.sprintf "U+%04X") |> String.concat "; "

(* RFC 3492 Section 7.1: "output MUST be all lowercase" *)
let check_encode cps expected =
  match Idna.Punycode.encode cps with
  | Ok s ->
      Alcotest.(check string) (Printf.sprintf "encode → %S" expected) expected s
  | Error e -> Alcotest.fail (Printf.sprintf "encode: %s" e)

let check_encode_err cps =
  match Idna.Punycode.encode cps with
  | Error _ -> ()
  | Ok s ->
      Alcotest.fail
        (Printf.sprintf "expected encode error for [%s], got %S"
           (string_of_cps cps) s)
  | exception exn ->
      Alcotest.fail
        (Printf.sprintf "expected encode Error for [%s], got exception %s"
           (string_of_cps cps) (Printexc.to_string exn))

let rec repeat_cp cp n = if n <= 0 then [] else cp :: repeat_cp cp (n - 1)
...
let check_roundtrip encoded =
  ...
  | Ok cps -> (
      match Idna.Punycode.encode cps with
      | Error e ->
          Alcotest.fail (Printf.sprintf "encode after decode %S: %s" encoded e)
      | Ok result ->
...
let pc_h =
  "989aomsvi5e83db1d2a355cv1e0vak1dwrv93d5xbh15a0dtl30a5jpsd879ccm6fea98c"
...
let test_decode_all () =
  List.iter (fun (_name, cps, pc) -> check_decode pc cps) all_vectors
...
let test_roundtrip_all () =
  List.iter (fun (_, _, pc) -> check_roundtrip pc) all_vectors
  ...
  List.iter
    (fun (name, cps, _) ->
      ...
      | Ok encoded -> (
          match Idna.Punycode.decode encoded with
          | Error e ->
              Alcotest.fail (Printf.sprintf "decode after encode %s: %s" name e)
          | Ok result ->
              let expected = List.map lower_cp cps in
              Alcotest.(check (list int))
                (Printf.sprintf "enc→dec %s" name)
                expected result))
    all_vectors
...
let test_decode_overflow () =
  ...
  check_decode_err (String.make 200 'z')

let test_decode_surrogate_scalar_values () =
  check_decode_err "ib9b";
  check_decode_err "zy0c"
...
let test_decode_out_of_range () =
  ...
  let candidates = [ "zzzzzzzzz"; "y-zzzzzzzz"; "a-zzzzzzzzzz" ] in
  List.iter
    (fun input ->
      ...
      | Ok cps ->
          List.iter
            (fun cp ->
              if cp > 0x10FFFF then
                Alcotest.fail
                  (Printf.sprintf "%s: decoded U+%X > U+10FFFF" input cp))
            cps)
    candidates

(** Non-basic byte in basic segment (before delimiter) should be rejected. RFC
    3492 §5: basic codepoints must be <= 0x7F. Decoder that silently accepts
    high bytes produces out-of-spec output. *)
      ...
      | Ok cps -> (
          ...
          | Ok reencoded ->
              ...
              if lower_ace <> lower_reenc then
                Alcotest.fail
                  (Printf.sprintf "asymmetry: %s → %s" ace reencoded)))
    valid_a_labels
...
let test_encode_rejects_non_scalar_values () =
  check_encode_err [ -1 ];
  check_encode_err [ 0xD800 ];
  check_encode_err [ 0xDFFF ];
  check_encode_err [ 0x110000 ]

let test_surrogate_alabel_rejected_by_high_level_paths () =
  let label = "xn--ib9b" in
  (match Idna.Registration.check_label label with
  | Error _ -> ()
  | Ok () ->
      Alcotest.fail
        (Printf.sprintf "Registration.check_label accepted %S" label));
  (match Idna.Lookup.to_ascii label with
  | Error _ -> ()
  | Ok value ->
      Alcotest.fail
        (Printf.sprintf "Lookup.to_ascii accepted %S as %S" label value));
  let unicode = Idna.Uts46.to_unicode label in
  Alcotest.(check bool) "Uts46.to_unicode errored" true unicode.errored;
  match Idna.Uts46.to_ascii label with
  | Error _ -> ()
  | Ok value ->
      Alcotest.fail
        (Printf.sprintf "Uts46.to_ascii accepted %S as %S" label value)

let test_encode_repeated_max_scalar_is_valid () =
  check_encode (repeat_cp 0x10FFFF 16) "dn32gaaaaaaaaaaaaaaa"

(* ══════════════════════════════════════════════════════════════ *)
(* 9. Encode: word-size overflow boundary (RFC 3492 §6.4)        *)
(* ══════════════════════════════════════════════════════════════ *)

let test_encode_word_size_overflow_boundary () =
  let cps = repeat_cp 0x61 963 @ [ 0x10FFFF ] in
  match (Sys.word_size, Idna.Punycode.encode cps) with
  | 32, Error _ -> ()
  | 32, Ok s ->
      Alcotest.fail (Printf.sprintf "expected 32-bit encode overflow, got %S" s)
  | _, Ok _ -> ()
  | _, Error e ->
      Alcotest.fail
        (Printf.sprintf "expected non-32-bit encode success, got %s" e)
    ...
    [
      ...
      ( "decode:negative",
        [
          ...
          Alcotest.test_case "surrogate scalar values" `Quick
            test_decode_surrogate_scalar_values;
          Alcotest.test_case "out of range" `Quick test_decode_out_of_range;
          Alcotest.test_case "non-basic in basic" `Quick
            test_decode_non_basic_in_basic;
          Alcotest.test_case "trailing separator" `Quick
            test_decode_trailing_separator;
      ...
      ( "encode:boundary",
        [
          ...
          Alcotest.test_case "single nonbasic" `Quick
            test_encode_single_nonbasic;
          Alcotest.test_case "single basic" `Quick test_encode_single_basic;
          Alcotest.test_case "repeated max scalar" `Quick
            test_encode_repeated_max_scalar_is_valid;
        ] );
      ( "decode:digits",
        [
      ...
      ( "encode:negative",
        [
          Alcotest.test_case "non-scalar values" `Quick
            test_encode_rejects_non_scalar_values;
          Alcotest.test_case "word-size overflow boundary" `Quick
            test_encode_word_size_overflow_boundary;
        ] );
      ( "high-level a-label",
        [
          Alcotest.test_case "surrogate a-label rejected" `Quick
            test_surrogate_alabel_rejected_by_high_level_paths;
        ] );
    ]
