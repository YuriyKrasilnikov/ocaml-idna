let check_ascii_ok ?(flags = Idna.Uts46.default_flags) input expected =
  ...
  | Error msg ->
      Alcotest.fail
        (Printf.sprintf "Uts46.to_ascii %S: expected Ok %S, got Error %S" input
           expected msg)
...
let () =
  Alcotest.run "generated"
    [
      ( "uts46 deviation",
        [
          Alcotest.test_case "sharp s preserved" `Quick
            test_deviation_behavior_sharp_s;
          Alcotest.test_case "final sigma preserved" `Quick
            test_deviation_behavior_final_sigma;
          Alcotest.test_case "zwnj preserved" `Quick
            test_deviation_behavior_zwnj_preserved;
          Alcotest.test_case "zwj preserved" `Quick
            test_deviation_behavior_zwj_preserved;
        ] );
    ]
